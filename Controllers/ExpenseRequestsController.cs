...
{
    private readonly IExpenseRequestsService _service;

    public ExpenseRequestsController(IExpenseRequestsService service)
    {
        _service = service;
    }
    ...
    public async Task<IActionResult> Create([FromForm] CreateExpenseRequestDto dto)
    {
    ...
    {
        var result = await _service.GetAttachmentAsync(id, BuildUserContext());
        if (!result.Success)
            return ToErrorResult(result.StatusCode ?? 400, result.Message);
